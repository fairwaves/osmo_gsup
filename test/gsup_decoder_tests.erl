-module(gsup_decoder_tests).
-include_lib("eunit/include/eunit.hrl").

-define(setup(F), {setup, fun start/0, fun stop/1, F}).


-define(BINARY_INSERT_SUBSCRIBER_DATA_REQUEST, <<16,1,8,98,66,130,119,116,88,81,242,8,7,6,148,97,49,100,96,33,5,7,16,1,1,18,2,1,42,40,1,1>>).
...
-define(BINARY_MO_FORWARD_SM_REQUEST, <<36,1,8,98,66,2,0,0,0,128,248,64,1,66,65,5,3,0,137,103,245,66,8,2,6,148,33,3,0,0,136,67,10,5,35,5,0,33,67,245,0,0,0>>).
...
-define(BINARY_SUPPLEMENTARY_SERVICE_REQUEST, <<32,1,8,98,66,2,0,0,0,64,246,48,4,32,0,0,1,49,1,1,53,21,161,19,2,1,5,2,1,59,48,11,4,1,15,4,6,170,81,12,6,27,1>>).
...
check_insert_subscriber_data_request(_Pid) ->
  Map = gsup_decoder:decode(?BINARY_INSERT_SUBSCRIBER_DATA_REQUEST),
  [?_assertEqual({?MAP_INSERT_SUBSCRIBER_DATA_REQUEST, []}, Map)].

check_mo_forward_sm_request(_Pid) ->
  Map = gsup_decoder:decode(?BINARY_MO_FORWARD_SM_REQUEST),
  [?_assertEqual({?MAP_MO_FORWARD_SM_REQUEST, []}, Map)].

check_supplementary_service_request(_Pid) ->
  Map = gsup_decoder:decode(?BINARY_SUPPLEMENTARY_SERVICE_REQUEST),
  [?_assertEqual({?MAP_SUPPLEMENTARY_SERVICE_REQUEST, []}, Map)].

check_send_auth_info_result(_Pid) ->
  Map = gsup_decoder:decode(?BINARY_SEND_AUTH_INFO_RESULT),
  [?_assertEqual({?MAP_SEND_AUTH_INFO_RESULT, []}, Map)].
